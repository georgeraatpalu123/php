...
$nimed = array("Mari", "Marju", "Karin", "Anna","Emily","Elis","Kati", "Mari","Laura" );

sort($nimed);


echo "Tüdrukute nimed kasvavas järjekorras:<br>";
foreach ($nimed as $nimi) {
    echo $nimi . "<br>";
}

$esimesed_kolm = array_slice($nimed, 0, 3);
echo "<br>Esimene kolm nime:<br>";
foreach ($esimesed_kolm as $nimi) {
    echo $nimi . "<br>";
}


$viimane_nimi = end($nimed);
echo "<br>Viimane nimi:<br>" . $viimane_nimi . "<br>";


$suvaline_indeks = rand(0, count($nimed) - 1);
$suvaline_nimi = $nimed[$suvaline_indeks];
...
$toyota = array_count_values($autod)["Toyota"];
$audi = array_count_values($autod)["Audi"];

echo "Toyotade arv: " . $toyota . "<br>";
echo "Audide arv: " . $audi . "<br>";
